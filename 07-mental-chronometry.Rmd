...
quantilefun <- function(x) {
  as_tibble(t(quantile(x$RT, probs = deciles)))
}
...
df_quantile_differences <- df_quantiles %>% 
  ...
  mutate(Differenz = goNoGo - discrimination)
```


```{r}
df_quantile_differences %>% 
  ggplot(aes(x = goNoGo, y = Differenz)) +
  geom_line(aes(group = 1), color = "steelblue", size = 2) +
  geom_point(shape = 21, color = "steelblue", fill = "white", size = 3, stroke = 3) +
