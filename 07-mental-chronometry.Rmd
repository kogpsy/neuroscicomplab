...
Eine einfachere Variante ist mit der neuesten Version von `summarise` möglich. `quibble` ist hier eine selber definierte Funktion, welche die Quantile berechnet und ein einem `tibble` speichert. 
...
Wir müssen anschliessend den Dataframe ins "wide" Format konvertieren, um zwei Spalten für die "goNoGo" und "discrimination" Bedingungen zu erhalten, und können danach die Differenz berechnen.
...
Mit dem `rogme` (Robust Graphical Methods For Group Comparisons) Package lassen sich solche Shift Functions ohne grossen Aufwand für Gruppenvergleiche grafisch darstellen. Dies ist sowohl für unabhängige Gruppen, als auch abhängige.
...
Mit Funktion aus diesem Package können wir Shift Functions für unabhängige Gruppen, sowie messwiederholte Bedingungen berechnen. Für den Vergleich zweier messwiederholter Tasks in nur einer Person benutzen wir die Funktion `shiftdhd_pbci()`, welche die Quantildifferenzen und Konfodenzintervalle berechnet.
