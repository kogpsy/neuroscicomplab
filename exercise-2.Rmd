...
nback1 <- read_csv("data/adhd-nback.csv")
```
...
:::exercise
Konverieren Sie die Gruppierungsvariable `group` zu einem Faktor:
:::
...
Veräandern Sie die Reihenfolgen der Faktorstufen:
...
d <- read_csv("data/cognitive-load-nback.csv") %>% 
...
Wir wollen nun die **population-level** Effects mit `brms` schätzen, und zwar die Mittelwerte der beiden Cognitive Load Bedingungen. Dafür benutzen wir die Formel `~ 0 + load`. Bevor wir anfangen, ändern wir die `rt` Variable von Millisekunden zu Sekunden:
...
Die beiden flachen Priors der Parameter `loadhi` unf `loadlo` wollen wir nun durch eigene Priors ersetzen. Wir nehmen hier einfach Normalverteilungen, mit einem Mittelwert, der dem Gesamtmittelwert entspricht, und einer etwas arbiträr gewählten Standardabweichung von $10$.

```{r}
grandmean <- round(mean(d$rt), 0)
grandmean
```
...
:::exercise
Überprüfen Sie, ob die `Rhat` Werte in Ordnung sind. Falls ja, fassen Sie die Posterior Verteilung der wichtigen Parameter zusammen. Stellen Sie die Posterior Verteilungen der beiden Bedingungen grafisch dar `mcmc_plot()`.
:::
