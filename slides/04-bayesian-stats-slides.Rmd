...
Wir fangen mit einem simplen Beispiel an.
...
## Verlgeich mit No Pooling: Shrinkage
...
person_effects %>% 
  ...
  stat_halfeye() +
  geom_vline(xintercept = fixef(m_part_pool)[1, 1], color = "red", size = 1) +
  geom_vline(xintercept = fixef(m_no_pool)[1, 1], color = col[1], size = 1) +
  geom_vline(xintercept = fixef(m_no_pool)[2, 1], color = col[2], size = 1) +
   geom_vline(xintercept = fixef(m_no_pool)[3, 1], color = col[3], size = 1) +
...
dwide <- tibble(id = factor(1:10), 
  ...
  mutate(diff = post - pre,
         id = as_factor(id), 
         treat =  factor(treat, levels = c("control", "treat")))
```
]
...
- Wir haben einen `between-subjects` Faktor: `treat`
...
```{r echo=TRUE, eval=FALSE}
d %>% 
  ggplot(aes(time, score, color = treat)) +
...
```{r}
library(lme4)
lme_model <- lmer(score ~ treat * time + (1|id), 
                  data = d)
```
...
priors <- get_prior(score ~ treat*time,
          data = d)
